        ...
        try {
            ...
            for (CardTerminal reader : do_readers) {
                ...
                try {
                    ...
                    GlobalPlatform gp = new GlobalPlatform(new CardChannelInterface() {
                        @Override
                        public pro.javacard.gp.smardcardio.Card getCard() {
                            return null;
                        }

                        @Override
                        public int getChannelNumber() {
                            return card.getBasicChannel().getChannelNumber();
                        }

                        @Override
                        public pro.javacard.gp.smardcardio.ResponseAPDU transmit(pro.javacard.gp.smardcardio.CommandAPDU var1) throws pro.javacard.gp.smardcardio.CardException {
                            try {
                                return new pro.javacard.gp.smardcardio.ResponseAPDU(card.getBasicChannel().transmit(new javax.smartcardio.CommandAPDU(var1.getBytes())).getBytes());
                            } catch (CardException e) {
                                throw new pro.javacard.gp.smardcardio.CardException(e);
                            }
                        }

                        @Override
                        public int transmit(ByteBuffer var1, ByteBuffer var2) throws pro.javacard.gp.smardcardio.CardException {
                            try {
                                return card.getBasicChannel().transmit(var1, var2);
                            } catch (CardException e) {
                                throw new pro.javacard.gp.smardcardio.CardException(e);
                            }
                        }
                        ...
                        @Override
                        public void close() throws pro.javacard.gp.smardcardio.CardException {
                            try {
                                card.getBasicChannel().close();
                            } catch (CardException e) {
                                throw new pro.javacard.gp.smardcardio.CardException(e);
                            }
                        }
                    ...
                    if (needsAuthentication(args)) {
                        ...
                        if (args.has(OPT_SECURE_APDU)) {
                            for (Object s : args.valuesOf(OPT_SECURE_APDU)) {
                                CommandAPDU c = new CommandAPDU((byte[]) s);
                                gp.transmit(new pro.javacard.gp.smardcardio.CommandAPDU(c.getBytes()));
                            }
                        }
